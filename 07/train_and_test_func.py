...
def training_step(model, input, target, loss_function, optimizer):
    ...
    with tf.GradientTape() as tape:
        prediction = model(input, training=True)[-1]
...
def run_training_and_testing(model,
    ...
    """
    Trains and tests the model with the given training and testing datasets, optimizer, and loss function
    for the number of epochs. Plots train and test losses as well as test accuracy for all epochs.

    :param tf.keras.layers.Layer model: The model to be trained and tested.
    :param tf.data.Dataset train_ds: The training dataset.
    :param tf.data.Dataset test_ds: The testing dataset.
    :param tf.keras.optimizers.Optimizer optimizer: The optimizer used for training the model.
    :param tf.keras.losses.Loss loss_func: The loss function used in training/testing the model.
    :param int epochs: The number of epochs the model should be trained for.
    """
